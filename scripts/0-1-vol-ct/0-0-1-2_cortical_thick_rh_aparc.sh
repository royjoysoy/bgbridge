...
rm ct_temp_rh_aparc.txt
rm ct_output_rh_aparc.txt
file_list=($(cat rh_aparc_path.txt)) 


touch ct_output_rh_aparc.txt
for subj in "${file_list[@]}"
do
	volume=$(grep -v "^#" $subj | awk '{print $5}')
	#echo -e "${subj:38:47}\n$volume" > temp_lh_aparc.txt
    echo -e "${subj#*sub-}\n$volume" | cut -d/ -f1 > ct_temp_rh_aparc.txt
	paste ct_temp_rh_aparc.txt ct_output_rh_aparc.txt > ct_pasted_rh_aparc.txt
	mv ct_pasted_rh_aparc.txt ct_output_rh_aparc.txt
 	#echo -e "${subj:5:7}\n$volume" > temp.csv
