# 12-20-24 Fri Korea Roy Seo
# 6-1-nn_regression_standardscaler_aug_LanControlRegionsOnlymultivariate_12_17_24.py 에서
# 모델 단순화, early stopping추가, L2 regularization을 순차적으로 추가해 본 뒤에
# 이파일에서는 
# 1. L2 regularization 강도조정 : 더 약한 규제 -kernel_regularizer=l2(0.001)  더 강한 주제 kernel_regularizer=l2(0.05)
# 2. Learning Rate 조정 optimizer=Adam(learning_rate=0.001에서 0.0001로 조정) 
# 3. 추가적인 모델 단순화
...
def build_model(input_dim):
    model = Sequential([
        Dense(32, activation='relu', input_dim=input_dim, kernel_regularizer=l2(0.01)),
        Dropout(0.3),
...
# def build_model(input_dim):
#     model = Sequential([
#         Dense(64, activation='relu', input_dim=input_dim, kernel_regularizer=l2(0.01)), # L2 regularization
#         Dropout(0.4),
...
# def build_model(input_dim):
#     model = Sequential([
#         Dense(64, activation='relu', input_dim=input_dim), # reduced from 128 to 64
#         Dropout(0.4),                                      # increased dropout
#         Dense(32, activation='relu'),                      # removed middle layer 
#         Dropout(0.3),
#         Dense(len(output_NPT), activation='linear')
#     ])
